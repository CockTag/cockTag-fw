...
#define PERIPHERAL_LINK_COUNT \
  0 /**< Number of peripheral links used by the application. When changing this number remember to adjust the RAM settings*/

#define IS_SRVC_CHANGED_CHARACT_PRESENT \
  0 /**< Include or not the service_changed characteristic. if not enabled, the server's database cannot be changed for the lifetime of the device*/

#define APP_CFG_NON_CONN_ADV_TIMEOUT \
  0 /**< Time for which the device must be advertising in non-connectable mode (in seconds). 0 disables timeout. */
#define NON_CONNECTABLE_ADV_INTERVAL \
  MSEC_TO_UNITS(5000, UNIT_0_625_MS) /**< T
    ...
    offline_finding_adv_template[] =
        {
            ...
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, /* First two bits */
            0x00,                                     /* Hint (0x00) */
};
...
void set_addr_from_key(uint8_t* key) {
  ...
  ble_gap_addr_t address = {.addr_type = BLE_GAP_ADDR_TYPE_RANDOM_STATIC};
  ...
  NRF_LOG_DEBUG("address is: %X:%X:%X:%X:%X:%X\n", address.addr[5],
...
static void set_key(uint8_t* key) {
  ...
  ret_code_t err_code =
      sd_ble_gap_adv_data_set(offline_finding_adv_template,
                              sizeof(offline_finding_adv_template), NULL, 0);
...
void timers_init() {
  ...
  app_timer_create(&key_cycle_timer, APP_TIMER_MODE_REPEATED,
                   key_cycle_timer_handler);
  app_timer_create(&led_blink_timer, APP_TIMER_MODE_REPEATED,
                   led_timer_handler);
...
void button_handler(long unsigned int pin, nrf_gpiote_polarity_t polarity) {
  ...
  ignore_button = true;
  ret_code_t err_code;
  err_code = app_timer_start(debounce_timer, DEBOUNCE_PERIOD, NULL);
  APP_ERROR_CHECK(err_code);
...
void buttons_init() {
  ...
  nrf_drv_gpiote_in_config_t in_config = {
      .sense = BUTTON_PULL == NRF_GPIO_PIN_PULLUP ? NRF_GPIOTE_POLARITY_HITOLO
